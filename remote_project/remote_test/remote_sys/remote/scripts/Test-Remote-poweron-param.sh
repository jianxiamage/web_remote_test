...
retCode=0
#----------------------------------------------------------------
cmdEndStr="Remote Test:start (reset) the remote node End-------------------------"
DebugerIP=''
DebugerUser='root'
...
write_log "INFO" "Remote Test:start (reset) the remote node Begin-----------------------"
echo "Remote Test:start the remote node..."

if [ $# -ne 2 ];then
    echo "Parameter error,usage:Test-Remote-poweron-param.sh DebugerIP cmdLine"
    exit 1
fi
